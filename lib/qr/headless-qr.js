	...
	for (let r = -1; r <= 7; r += 1) {
		...
		for (let c = -1; c <= 7; c += 1) {
			...
			if (
				...
				modules[row + r][col + c] = ["probe", true];
			} else {
				modules[row + r][col + c] = ["probe", false];
			}
		}
	}
}
			...
			for (let r = -2; r <= 2; r += 1) {
				for (let c = -2; c <= 2; c += 1) {
					if (r == -2 || r == 2 || c == -2 || c == 2 || (r == 0 && c == 0)) {
						modules[row + r][col + c] = ["adjust", true];
					} else {
						modules[row + r][col + c] = ["adjust", false];
					}
				}
			}
		}
...
function setup_type_info(modules, test, mask_pattern, error_correction_level) {
	...
	for (let i = 0; i < 15; i += 1) {
		...
		if (i < 6) {
			modules[i][8] = ["info", mod];
		} else if (i < 8) {
			modules[i + 1][8] = ["info", mod];
		} else {
			modules[modules.length - 15 + i][8] = ["info", mod];
		}
	}
	...
	for (let i = 0; i < 15; i += 1) {
		...
		if (i < 8) {
			modules[8][modules.length - i - 1] = ["info", mod];
		} else if (i < 9) {
			modules[8][15 - i - 1 + 1] = ["info", mod];
		} else {
			modules[8][15 - i - 1] = ["info", mod];
		}
	}

	// fixed module
	modules[modules.length - 8][8] = ["info", !test];
}
...
function setup_version_number(modules, version, test) {
	...
	for (let i = 0; i < 18; i += 1) {
		const mod = !test && ((bits >> i) & 1) == 1;
		modules[Math.floor(i / 3)][(i % 3) + modules.length - 8 - 3] = ["version", mod];
	}

	for (let i = 0; i < 18; i += 1) {
		const mod = !test && ((bits >> i) & 1) == 1;
		modules[(i % 3) + modules.length - 8 - 3][Math.floor(i / 3)] = ["version", mod];
	}
}
